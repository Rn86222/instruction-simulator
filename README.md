...
- `--ppm` オプションは、レイトレの結果を出力する ppm ファイル名を指定します。指定しない場合は、バイナリファイルの名前の `.bin` を `.ppm` に置き換えたものが使われます。`outchar` や `outint` 命令が実行されるたびに、対応するレジスタの値がこのファイルに出力されていきます。
...
- `--show-output` オプションは、`outchar` や `outint` 命令によって出力された文字を、実行終了時に標準出力に表示するものです。これをセットするかどうかにかかわらず、ppm ファイルには出力されます。
...
#### 実行時間予測について
実行時間予測について説明しておきます。まず実行サイクル数は次の式で予測されます。
$ (実行サイクル数) = (実行命令数) + (キャッシュヒット時のストール数) + (キャッシュミスによるストールの合計サイクル数) + (flush によるオーバーヘッドの合計サイクル数) + (FPUのストールの合計サイクル数) $
これを動作周波数で割ることで実行時間がわかりそうですが、UART通信の影響が大きいことがわかり、それだけでは実行時間が正確に予測できないことがわかりました。そこで、16x16、32x32、64x64、128x128の場合の実行時間を計測し、またそれらの場合における実行サイクル数とppmファイルのサイズとの関係を線形回帰(Python の scikit-learn)により調べました。その結果、次の式が得られました。
$ (実行時間) = (実行サイクル数) \times 1.59512149\times 10 ^{-8} + (出力バイト数) \times 2.72652771 \times 10^{-5} + 121.06771803862078 $
これを用いて、256x256の実行時間を予測すると、392.19sという結果になりました。一方実際の実行時間は392.1510sとなりました。誤差は0.0039sで、0.01%未満です。
