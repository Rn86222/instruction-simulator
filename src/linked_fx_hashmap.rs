...
/// A linked hash map.
pub struct LinkedFxHashMap<K, V> {
    map: FxHashMap<KeyRef<K>, *mut Node<K, V>>,
    head: *mut Node<K, V>,
    free: *mut Node<K, V>,
}
...
impl<K: Hash + Eq, V> LinkedFxHashMap<K, V> {
    ...
    pub fn new() -> Self {
        Self::with_map(FxHashMap::default())
    }

    // Creates an empty linked hash map with the given initial capacity.
    // pub fn with_capacity(capacity: usize) -> Self {
    //     Self::with_map(FxHashMap::with_capacity(capacity))
    // }
}

impl<K, V> LinkedFxHashMap<K, V> {
    #[inline]
...
impl<K: Hash + Eq, V> LinkedFxHashMap<K, V> {
    fn with_map(map: FxHashMap<KeyRef<K>, *mut Node<K, V>>) -> Self {
        LinkedFxHashMap {
            map,
            head: ptr::null_mut(),
            free: ptr::null_mut(),
    ...
    //     Self::with_map(FxHashMap::with_hasher(hash_builder))
    // }

    /// Creates an empty linked hash map with the given initial capacity and hash builder.
    // pub fn with_capacity_and_hasher(capacity: usize, hash_builder: S) -> Self {
    //     Self::with_map(FxHashMap::with_capacity_and_hasher(capacity, hash_builder))
    // }
...
impl<'a, K, V, Q: ?Sized> Index<&'a Q> for LinkedFxHashMap<K, V>
where
    K: Hash + Eq + Borrow<Q>,
    Q: Eq + Hash,
{
...
impl<'a, K, V, Q: ?Sized> IndexMut<&'a Q> for LinkedFxHashMap<K, V>
where
    K: Hash + Eq + Borrow<Q>,
    Q: Eq + Hash,
{
...
// impl<K: Hash + Eq + Clone, V: Clone> Clone for LinkedFxHashMap<K, V> {
//     fn clone(&self) -> Self {
...
impl<K: Hash + Eq, V> Extend<(K, V)> for LinkedFxHashMap<K, V> {
...
impl<'a, K, V> Extend<(&'a K, &'a V)> for LinkedFxHashMap<K, V>
where
    K: 'a + Hash + Eq + Copy,
    V: 'a + Copy,
{
...
// impl<K: Hash + Eq, V> iter::FromIterator<(K, V)> for LinkedFxHashMap<K, V> {
...
impl<A: fmt::Debug + Hash + Eq, B: fmt::Debug> fmt::Debug for LinkedFxHashMap<A, B> {
...
impl<K: Hash + Eq, V: PartialEq> PartialEq for LinkedFxHashMap<K, V> {
    fn eq(&self, other: &Self) -> bool {
        self.len() == other.len() && self.iter().eq(other)
    }
}

impl<K: Hash + Eq, V: Eq> Eq for LinkedFxHashMap<K, V> {}

impl<K: Hash + Eq + PartialOrd, V: PartialOrd> PartialOrd for LinkedFxHashMap<K, V> {
...
impl<K: Hash + Eq + Ord, V: Ord> Ord for LinkedFxHashMap<K, V> {
    fn cmp(&self, other: &Self) -> Ordering {
        self.iter().cmp(other)
    }
}

impl<K: Hash + Eq, V: Hash> Hash for LinkedFxHashMap<K, V> {
...
unsafe impl<K: Send, V: Send> Send for LinkedFxHashMap<K, V> {}

unsafe impl<K: Sync, V: Sync> Sync for LinkedFxHashMap<K, V> {}

impl<K, V> Drop for LinkedFxHashMap<K, V> {
    fn drop(&mut self) {
        if !self.head.is_null() {
            unsafe {
                self.drop_entries();
...
impl<'a, K, V> Iterator for Entries<'a, K, V> {
    type Item = OccupiedEntry<'a, K, V>;

    fn next(&mut self) -> Option<OccupiedEntry<'a, K, V>> {
        if self.remaining == 0 {
            None
        } else {
            self.remaining -= 1;
...
impl<'a, K: Hash + Eq, V> IntoIterator for &'a LinkedFxHashMap<K, V> {
    type Item = (&'a K, &'a V);
...
impl<'a, K: Hash + Eq, V> IntoIterator for &'a mut LinkedFxHashMap<K, V> {
    type Item = (&'a K, &'a mut V);
...
impl<K: Hash + Eq, V> IntoIterator for LinkedFxHashMap<K, V> {
    type Item = (K, V);
